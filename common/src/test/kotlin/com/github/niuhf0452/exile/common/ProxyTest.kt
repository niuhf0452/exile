...
abstract class ProxyTest : FunSpec() {
    ...
    init {
            ...
            val factory = newBuilder<AtomicInteger>()
                    ...
                    .handle { m ->
                        if (m is KProperty.Getter) {
                            object : AsyncMethodHandler<AtomicInteger>() {
                                override val function: KFunction<*>
                                    get() = m

                                override suspend fun asyncCall(state: AtomicInteger, instance: Any, args: Array<out Any?>?): Any? {
                                    return state.get()
                                }
                            }
                        } else {
        ...
        test("A proxy should throw if not given interface") {
            shouldThrow<IllegalArgumentException> {
                newBuilder<Unit>().build()
            }
        }
        ...
        test("A proxy should handle coroutine method") {
            val a = newBuilder<Unit>()
                    .addInterface(Test7::class)
                    .handle { m ->
                        object : AsyncMethodHandler<Unit>() {
                            override val function: KFunction<*>
                                get() = m

                            override suspend fun asyncCall(state: Unit, instance: Any, args: Array<out Any?>?): Any? {
                    ...
                    .build()
                    .createObject(Unit) as Test7
            a.foo(123) shouldBe "abc"
        }
    }
